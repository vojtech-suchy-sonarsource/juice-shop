...
function calculateAccuracy (challengeKey: string, phase: Phase) {
  ...
  let coloredAccuracy
  if (accuracy > 0.5) {
    coloredAccuracy = colors.green(accuracy.toString())
  } else if (accuracy > 0.25) {
    coloredAccuracy = colors.yellow(accuracy.toString())
  } else {
    coloredAccuracy = colors.red(accuracy.toString())
  }
  logger.info(`Accuracy for '${phase === 'fix it' ? 'Fix It' : 'Find It'}' phase of coding challenge ${colors.cyan(challengeKey)}: ${coloredAccuracy}`)
  return accuracy
}
